...
// testBulkLOBStmtNo tests that HDB error statement numbers are offset per
// package when a bulk insert is split into pieces according to partial LOBs
// (M2): the failing row is the second package's last row.
func testBulkLOBStmtNo(t *testing.T, ctr *Connector, db *sql.DB) {
	table := RandomIdentifier("bulkInsertLOBStmtNo")

	if _, err := db.ExecContext(t.Context(), fmt.Sprintf("create table %s (k integer primary key, c clob)", table)); err != nil {
		t.Fatalf("create table failed: %s", err)
	}

	// insert 1 row
	if _, err := db.ExecContext(t.Context(), fmt.Sprintf("insert into %s values (3, 'small')", table)); err != nil {
		t.Fatalf("pre-insert failed: %s", err)
	}

	// lob streaming is not permitted in auto-commit mode - so insert in a transaction
	...
	// insert 3 rows (k: 1,2,3) - the first row has a lob bigger than the lob
	...
	// check, that StmtNo matches the duplicate row index in the whole bulk
	if hdbErr.StmtNo() != 2 {
		t.Fatalf("actual StmtNo %d - expected StmtNo %d", hdbErr.StmtNo(), 2)
	}
}

func testBulkGeo(t *testing.T, ctr *Connector, db *sql.DB) {
	const (
