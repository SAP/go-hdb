...
func TestJWTAuth(t *testing.T) {
	stepper := newAuth(&SessionConfig{Token: "dummy token"})
	...
	decStep := func(t *testing.T, data []byte) {
		auth, err := stepper.next()
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("step1", func(t *testing.T) {
		decStep(t, []byte("\x02\x00\x03JWT\x07USER123"))

		authJWT := stepper.methods.method.(*authJWT)
	...
	t.Run("step3", func(t *testing.T) {
		decStep(t, []byte("\x02\x00\x03JWT\x205be8f43e064e0589ce07ba9de6fce107"))
