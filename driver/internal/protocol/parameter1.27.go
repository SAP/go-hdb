...
func (f *ParameterField) decodeResult(dec *encoding.Decoder, attrs *ReaderAttrs) (any, error) {
	return decodeResult(f.tc, dec, attrs, f.scale)
}

func (p *OutputParameters) decodeResult(dec *encoding.Decoder, header *PartHeader, attrs *ReaderAttrs) error {
	...
	for i := range numArg {
		for j, f := range p.OutputFields {
			var err error
			if p.FieldValues[i*cols+j], err = f.decodeResult(dec, attrs); err != nil {
